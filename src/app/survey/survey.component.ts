...
export class SurveyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get(`http://localhost:3000/survey/${this.id}/${this.email}`).subscribe((response: any) => {
  ...
  submit() {
    ...
    this.http.post(`http://localhost:3000/survey/response`, surveyResponse).subscribe((response: any) => {
